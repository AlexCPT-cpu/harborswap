export const fee = process.env.NEXT_PUBLIC_FEE;
export const bearer = process.env.NEXT_PUBLIC_BEARER;
export const bearer1 = process.env.NEXT_PUBLIC_BEARER1;
export const bearer2 = process.env.NEXT_PUBLIC_BEARER2;
export const bearer3 = process.env.NEXT_PUBLIC_BEARER3;

export const FeeAddr = process.env.NEXT_PUBLIC_REFERRER

export const moralisKey = process.env.NEXT_PUBLIC_MORALIS_KEY;
export const moralisKey2 = process.env.NEXT_PUBLIC_MORALIS_KEY2;
export const moralisKey3 = process.env.NEXT_PUBLIC_MORALIS_KEY3;
export const moralisKey4 = process.env.NEXT_PUBLIC_MORALIS_KEY4;

export const bearerArray = [bearer, bearer1, bearer2, bearer3];
export const moralisArray = [moralisKey, moralisKey2, moralisKey3, moralisKey4];
export const oneInchContract = "0x1111111254EEB25477B68fb85Ed929f73A960582";
export const chainAddress = "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE";
export const chainId = 1;
